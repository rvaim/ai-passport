...
static bsp_btn_t       s_btn_id[BSP_BTN_COUNT] = {
    BSP_BTN_UP, BSP_BTN_DOWN, BSP_BTN_OK,
};
...
// 每个按键把"哪个键"随回调带回来。button 组件的回调签名固定,故用 usr_data 传枚举地址。
static void on_event(void *arg, void *usr_data, bsp_btn_ev_t ev) {
    (void)arg;
    if (!s_cb) return;
    s_cb(*(const bsp_btn_t *)usr_data, ev, s_user);
}
...
esp_err_t bsp_button_init(bsp_btn_cb_t cb, void *user) {
    ...
    for (int i = 0; i < BSP_BTN_COUNT; i++) {
        ...
        const button_config_t bc = {
            .long_press_time = i == BSP_BTN_OK ? 600U : 0U,
        };
    ...
    ESP_LOGI(TAG, "按键就绪:ADC1_CH%d 三键分压,UP/DOWN 按下即发,OK 长按 600ms",
             BSP_BTN_ADC_CHANNEL);
    return ESP_OK;
}
