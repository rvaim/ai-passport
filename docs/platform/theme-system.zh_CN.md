...
主题不能增加样式类型、改变继承树、替换字体、改变导航或按键语义、修改布局，也不能运行脚本。阴影影响的像素越多，混合成本越高，应使用能表达风格的最小尺寸。示例见[夜间主题](../../examples/themes/night/README.zh_CN.md)与[新粗野主义主题](../../examples/themes/neo-brutalism/README.zh_CN.md)。

## 主题生命周期

主题通过现有 `.pap` 包链路安装和更新。系统“主题”App 会列出内置默认主题与有效的已安装 Manifest。短按确定进入主题详情页，可应用或卸载选中的主题；卸载需要再次按确定确认，长按确定始终由系统导航组件负责返回。

卸载操作在共享存储工作任务中执行，会先把主题目录移动到 `.trash`，再递归删除。内置 `default` 主题受保护。如果删除的是当前已安装主题，系统会先应用并持久化 `default`，再排队删除，因此即使清理失败，设备也不会指向已不存在的主题。卸载主题不会影响 PAP 插件数据或全局插件数据配额。
