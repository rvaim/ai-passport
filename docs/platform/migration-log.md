...
New components are `passport_core`, `passport_ui`, `passport_link`, and `passport_runtime`, plus a new system launcher, package tools, authorization-panel sample, theme sample, and platform documentation. `passport_core` also owns the persistent brightness, volume, screen-timeout, and key-sound service.
