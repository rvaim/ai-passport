...
- 移除未发布阶段的 Manifest 与主题兼容路径：包现在必须使用当前完整 Schema 和 API `1`，会拒绝包括无效 `permissions` 占位字段在内的未知字段；主题必须以规范 `#RRGGBB` 提供全部 18 个 Token。设备会在提交安装目录前校验完整主题 Token，并用一套共享解析器和主机测试保持打包器与固件规则一致。
...
- 将可安装主题从 8 个配色/间距字段扩展为 18 个有界视觉 Token，新增列表表面、选中文字、边框、圆角和阴影几何。共享列表组件现会直接渲染这些 Token，并为绘制范围安全留边，不增加对象或任务；新粗野主义示例使用 2 px 边框与偏移 4 px 的硬阴影。
- 将原有的简短插件说明扩展为详细的双语插件开发指南，覆盖 Manifest 字段、生命周期、完整 Lua API、运行时限制、Link 行为、打包、安装、测试、排障和发布检查。
- 修复安装 `.pap` 导致设备复位：实测 `pap_install` 调用链进入 FATFS 与 cJSON 时会溢出原 4 KiB 栈。安装任务现使用 6 KiB 实测预算并报告最小剩余量；Registry 与主题 Manifest 缓冲区也移出系统任务栈。
- 将随仓库提供的计数器插件扩展为直接按键交互 Demo：上键增加、下键减少、中键清零，上、下键双击变化两次，计数限制在 -9999～9999；不持久化状态，也不创建后台任务。
