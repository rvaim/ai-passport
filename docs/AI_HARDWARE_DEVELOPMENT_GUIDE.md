...
app_main
  ...
  ├─ bsp_audio_init
  ├─ bsp_battery_init
  ├─ device_identity / plugin_installer / Nearby service
  ├─ app_registry（系统插件 + 下载插件）
  ├─ bsp_button_init(on_key)
  └─ LVGL Registry 主页
```

显示是 UI 的硬依赖，显示或 LVGL 初始化失败时 `app_main` 直接返回。按键、音频、电池是
软依赖：Registry 根据服务与插件权限标记不可用条目，其他功能继续运行。当前产品层、插件
运行时和开发规范分别见 `main/`、`components/plugin_runtime/` 和
...
不要为了校准恢复一套已删除的 demo 菜单，也不要只用理论分压值。
...
当前产品用 `plugin_host` 的常驻工作任务异步播放短方波；下载插件只有 `tone`，不能直接
访问 PCM 或麦克风。BSP 仍提供阻塞式录放音能力，新增录音产品功能时应采用可取消的分块
循环或流式存储，并设计明确的任务退出握手，不可一次分配长录音缓冲或假设存在 PSRAM。
...
- 芯片不应答时初始化返回 `ESP_ERR_NOT_FOUND`，状态栏显示未知电量，但整机继续运行。
...
- 插件安装任务、BLE 任务、设置/音效任务栈和最大连续空闲块；
...
1. 实现 `enter`、`exit`、`key` 生命周期；`enter` 创建并加载 screen，`exit` 先停任务/定时器，再删除 screen 并清空指针。
2. 在 `main/app_registry.c` 注册 descriptor，使用稳定 ID，并声明显示、存储、音频、电池、Nearby 或身份等所需服务。
3. 在 `main/CMakeLists.txt` 加入源文件；不要另建一份菜单或页面列表。
4. 产品页面文字使用简体中文并由字体生成测试覆盖；说明性注释可用中文或英文。慢操作放
   工作任务，UI 更新遵守 LVGL 锁规则。
5. 保留 OK 长按返回这一全局交互，不在插件中重复实现。

可下载功能使用 `.fpp` Manifest、固定预算 VM 和宿主服务。若现有宿主能力不足，应先扩展受限服务接口和资源预算，再添加对应指令；不要把任意 ESP-IDF 指针暴露给包代码。
...
环境验收标准是：`idf.py --version` 正确、`idf.py build` 无错误、设备可烧录、monitor 能
看到 `Passport plugin shell v2.4.0 starting`，并且启动后没有持续重启或 assert。
...
仓库提供统一主机测试，之后仍需完整构建 ESP-IDF 目标：
...
主机测试覆盖字体、包格式与 VM、设备码、插件管理模型、参考插件和 Python 工具；硬件变更
仍必须上板。
...
- Registry 可用 UP/DOWN 循环导航，OK 单击进入，OK 长按逐层返回。
- 某个可选外设故障只禁用依赖该服务的条目，不影响其他功能。
...
| ADC/按键 | 松开和三键实测 mV、快速连续 PRESS、OK click/long、不同电量下的裕量 |
| codec/I2S | 插件 tone 频率/速度；若修改录音 API，再用定向诊断验证非零录音、格式切换和取消 |
