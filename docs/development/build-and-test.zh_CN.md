...
静态主机测试会直接编译锁定的 Lua 源码，因此全新 checkout 在运行 `--static` 前必须先生成 Managed Components。激活 ESP-IDF 5.5.3 后运行 `idf.py -B build/static-deps -D "SDKCONFIG=$PWD/build/static-deps/sdkconfig" reconfigure`；CI 会自动执行相同的隔离依赖解析步骤。`managed_components/` 始终是生成目录，不得提交。
