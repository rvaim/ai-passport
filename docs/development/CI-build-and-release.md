...
4. In parallel, the Pages job uses GitHub's `actions/configure-pages` to read the configured Pages site, builds the dependency-free artifact directly from `web/installer.html`, `web/installer.mjs`, and `web/passport-install-protocol.mjs`, and uploads it.
