...
`.github/workflows/pipeline.yml` 是唯一的验证和发布工作流。它负责检查仓库与主机行为、构建 ESP32-C3 固件、发布固件 Release，并把一个共享 Web Bluetooth 设备工具部署到 GitHub Pages。工作流不会重新生成 PAP、构建 PAP 目录、提交 PAP，也不会发布仓库内的 PAP 文件。
...
1. 静态任务先使用 ESP-IDF 5.5.3 在隔离构建目录中生成 `dependencies.lock` 锁定的 Managed Components，再运行 `./tools/validate.sh --static`。共用门禁检查仓库规则、主机行为、`.pap` 打包器、Web Bluetooth 协议 helper、2FA PAP，以及准确的 Pages 产物。
...
4. Pages 任务与固件发布并行运行：它先使用 GitHub 官方 `actions/configure-pages` 读取已经配置的 Pages 站点，再从 `web/` 下的共享页面以及安装、Link 帧和 TOTP 协议模块构建并上传无依赖产物。
...
启用 Pages 后，公开工具位于 `https://rvaim.github.io/ai-passport/`。页面顶部的设备码连接由本地 `.pap` 安装器和 2FA 密钥发送工具共用。文件与账号数据只在浏览器和 Passport 之间传输，不会上传；TOTP 密钥沿用现有明文 Link 边界。Pages 产物不会内置 PAP 文件或包目录。Web Bluetooth 要求安全上下文以及桌面版 Chrome 或 Edge。
