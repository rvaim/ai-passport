...
- Removed unreleased manifest and theme compatibility paths: packages require the exact current schema and API `1`, unknown fields including the unused `permissions` placeholder are rejected, and themes use a bounded sparse `styles` object with no legacy `tokens` fallback. One shared parser and host suite keep packer and firmware rules aligned.
