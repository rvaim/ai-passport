...
This branch adds a Cordis-inspired plugin shell in which built-in capabilities and
downloaded packages share one registry and lifecycle. Plugins are installed with
Chrome Web Bluetooth after synchronizing the device's deterministic code; the
device itself never joins a network. Package format, tools, Flash layout, and
acceptance steps are documented in
[docs/PLUGIN_SYSTEM.md](docs/PLUGIN_SYSTEM.md). The JSON schema, instruction set,
permissions, UI/font rules, packaging workflow, debugging guide, and release
checklist are in
[docs/PLUGIN_DEVELOPMENT_GUIDE.md](docs/PLUGIN_DEVELOPMENT_GUIDE.md).

Host API v5 keeps raw canvas drawing optional while adding themed semantic
screens, list rows, value cards, a shared action bar, asynchronous confirmation
dialogs, runtime theme packages, and a system-owned Nearby gateway. Built-in pages, Counter, Settings, and
Meteor Tap use the same component contract; Meteor Tap deliberately keeps its
custom-drawn game area. Signed Settings plugins can control brightness,
output volume, key sound, screen timeout, theme selection, and the shared
device-information page. The pinned `system.settings` entry retains a built-in
fallback.
Communication plugins are foreground-only and can request device-code-gated
messages, blobs, and half-duplex voice. Exit and VM faults force system cleanup.
Installable plugins always render with the public 14 px GB2312 font. The packer
and firmware both reject package text outside that glyph set, so unsupported
characters cannot become blank boxes only after installation.
Previously authorized Passport devices are reconnected automatically by the web
installer; Chrome still requires its chooser for the first authorization.
...
Applications may also use ESP-IDF timers, FreeRTOS tasks, and internal Flash/NVS; the Pomodoro branch contains an NVS example. This branch initializes the installer BLE profile while the Plugins page is open, or the fixed Runtime GATT Gateway while a foreground plugin owns a `nearby` lease. Downloaded plugins never receive direct NimBLE, Wi-Fi, or socket access. The firmware does not start Wi-Fi, a SoftAP, or an HTTP server. `demo/claude-buddy-port` remains a BLE architecture reference, not a substitute for measuring the current board's antenna, RF performance, or power consumption. Every FoloToy AI Passport has 8 MB of Flash, and the default firmware configuration targets 8 MB.
...
To add a built-in capability, implement the `enter`, `exit`, and `key` lifecycle and register one `system_plugin_t` descriptor in `main/app_registry.c`. Declare every required service so the Registry can expose an unavailable state when a peripheral failed to initialize. Downloadable behavior should use the `.fpp` VM and host capabilities instead of adding a second menu.
...
- The default global interaction is `UP`/`DOWN` navigation, short `OK` to enter, and long `OK` to go back one level. A plugin without a back handler returns to the home page.
...
Run the complete host-side suite before building the ESP-IDF target:

```bash
tests/run_host_tests.sh
```

It verifies generated fonts, the package format and VM, device-code and plugin-manager
models, Nearby framing and ADPCM, reference plugins, and Python tooling. Different example branches may provide
additional commands; follow the README on that branch.
...
components/plugin_runtime/ Package, signature, slot store, installer, and bytecode VM
main/                    Product shell, Registry, system plugins, host, and LVGL UI
examples/plugins/        JSON plugin sources and signed reference packages
tools/                   Plugin, font, BLE, flash, and release utilities
tests/                   Host-side runtime, model, plugin, font, and tooling tests
docs/                    Hardware, plugin-system, and plugin-development documentation
