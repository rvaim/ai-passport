...
The installer is a local utility for sending one plug-in or theme `.pap` package from a desktop browser to one nearby Passport over Web Bluetooth. Both package types use the same page and transport path, and the selected package is never uploaded. The user enters the public pairing code shown on the Passport; the browser discovers devices by the Passport Service UUID in the primary advertisement, while the page reads the public Device Code from GATT after connecting and rejects a mismatched target.
